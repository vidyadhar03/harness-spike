...
- terms: 6 to 12 search terms an archive would index.
  - Include a small number of queries combining the defining relationship of the location brief (such as a shrine integrated with a large tree, roots enclosing masonry, or living root architecture) to capture how the key elements interact. Keep individual component searches (materials, individual construction techniques, terrain) as supporting references.
  ...
  - Only propose a term if you are reasonably confident it is a real, documented name or recognized phenomenon. Each term is checked against an encyclopedia and dropped if it does not exist, so a wrong guess costs nothing but an invented term you insist on wastes a slot.
